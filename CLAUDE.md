...
├── migrations/                   # sqlx migrations 0001-0004, applied on boot
...
- `/s/{ticker}`: symbol page — candlestick chart with indicators, key stats, fundamentals, filings
...
- **Fundamentals + filings — SEC EDGAR.** `company_tickers.json`, `companyfacts`, `submissions`; no key, a contact email (`SEC_CONTACT_EMAIL`) in the User-Agent. Stocks only — ETFs and indexes do not file.
